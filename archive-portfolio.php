<?php
/**
 * The template for displaying portfoio Archive.
 *
 * @package Thirty_One
...
<div id="primary" class="full_width">
	<div id="content" class="<?php echo $thirtyone_portfolio_columns ?>" role="main">
		...
		<button class="filters-button is-checked icon-all" data-filter="*" ><?php echo __( 'Show All' , 'thirtyone'); ?></button>
		  <?php 
		...
		<?php if (have_posts()) : ?>
			...
			<article class="isotope-item <?php echo get_the_term_list_text($post->ID, 'project-type', '', ' ',''); ?>" data-category="<?php echo get_the_term_list_text($post->ID, 'project-type', '', ', ',''); ?>">
				<header class="post-thumb portfolio_post_thumb clearfix">
				
					<a title="<?php print $title ?>" rel="lightbox" href="<?php the_permalink(); /* print portfolio_thumbnail_url($post->ID) */ ?>">
					...
					if ( 'one-column' == $thirtyone_portfolio_columns ) {
					 ...
					 ?>
					 <div class="overlay"><div class="overlay_text"><?php _e('View More.', 'thirtyone'); ?></div></div>
					</a>
